...
holds using using dedicated tools, such as proof assistants or dependently-typed prograamming languages. GPAC serves as an initial specification in which
...
expressing systems via explicit signal manipulation for closed feedback loops. Later publications addressed this issue~\cite{Lemos2022, EdilLemos2023} whilst introducing or keeping known problems, such as noisy and overloaded notation when using the DSL -- a consequence of an \textit{abstraction leaking} -- and
