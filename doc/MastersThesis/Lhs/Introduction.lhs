...
Chapter 2, \textit{Design Philosophy}, will explain basic Haskell concepts, such as the type system and different sorts of polymorphism, and it will bind them to numerical methods and GPAC's circuits. The next chapter, \textit{Effectful Integrals}, is dedicated to introduce GPAC's integrator representative in software, alongside further improvements in the overall modeling of physical systems. The follow-up chapter, \textit{Fixing Recursion} will introduce the fixed-point combinator, alongside its use in Haskell, to upgrade the so-far developed DSL, simplifying its design and usability. The next chapter, \textit{Execution Walkthrough}, will discuss how the proposed types aligns with mathematical definitions introduced in Chapter 2. Moreover, how to execute a simulation as well as a guided example are presented. At the end, some issues will be identified with the implementation at that point. Chapters 6 and 7, \textit{Travelling across Domains} and \textit{Caching the Speed Pill} respectively, address these concerns. Finally, limitations, future improvements and final thoughts are drawn in chapter 8, \textit{Conclusion}.
