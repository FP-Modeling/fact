...
The memoization added to \texttt{FACT} needs a second tweak, related to the executable models established in Chapter 4. The following code is the same example model used in that Chapter:
...
Due to the new type signature, this change implies changing the driver, i.e., modifying the function \textit{runCT} (the changes are analogus to the \textit{runCTFinal} function variant). Further, a new auxiliary function was created, \textit{subRunCT}, to separate the environment into two functions. The \textit{runCT} will execute the mapping with the function \textit{parameterize} and the auxiliary function will address the need for interpolation.
