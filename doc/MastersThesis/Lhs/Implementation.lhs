...
assume we \textit{aren't} using CT as the type alias and instead we will be showing the implementations as if we are using the definition used previously~\cite{Lemos2022} for the
\texttt{CT} type:
...
Aside from lifting operations, the final typeclass related to data manipulation is the \texttt{MonadIO} typeclass. It comprises only one function, \textit{liftIO}, and its purpose is to change the structure that is wrapping the value, going from an \texttt{IO} outer shell to the monad of interest, \texttt{CT} in this case. The usefulness of this typeclass will be more clear in the next topic, Section \ref{sec:integrator}. The implementation follows, alongside its visual representation in Figure \ref{fig:monadIO}. Once again, consider the explicit
...
required to use a compiler extension \texttt{FlexibleInstances}. Further, the same operations can be used as internal helpers for both versions of the type:
...
The simplest unit of all, \texttt{Constant Unit}, can be achieved via the implementation of the \texttt{Applicative} and \texttt{Num} typeclasses. First, this unit needs to receive the time of simulation at that point, which is granted by the \texttt{CT} type. Next, it needs to return a constant value $k$ for all moments in time. The \texttt{Num} given the \texttt{CT} type the option of using number representations, such as the types \texttt{Int}, \texttt{Integer}, \texttt{Float} and \texttt{Double}. Further, the \texttt{Applicative} typeclass can lift those number-related functions to the desired type by using the \textit{pure} function.
...
we create a new computation, so-called \texttt{z} --- a function wrapped in the \texttt{CT} type that receives a \texttt{Parameters} record and computes the result based on the solving method.
Because this computation needs to do lookups on some configuration values, we use the function \texttt{ask} (line 5) from \texttt{ReaderT} to get our environment values; in this case
a value of type \texttt{Parameters}. Later on, the follow-up step is to build a copy of the \textit{same process} being pointed by the \texttt{computation} pointer (line 6).
...
The preceding rules include defining connections with polynomial circuits --- an acyclic circuit composed only by constant functions, adders and multipliers. These special circuits are already being modeled in \texttt{FACT} by the \texttt{CT} type with a set of typeclasses, as explained in the previous Section about GPAC. The \textit{integrator functions}, e.g., \textit{readInteg} and \textit{updateInteg}, represent the composition rules.
