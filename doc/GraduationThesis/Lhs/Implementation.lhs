...
\end{center}
\end{minipage}
\caption{The typeclass \texttt{MonadIO} transforms a given value wrapped in \texttt{IO} into a different monad. In this case, the parameter \texttt{m} of the function is the output of the \texttt{Dynamics} type.}
\label{fig:monadIO}
\end{figure}

Finally, there are the typeclasses related to mathematical operations. The typeclasses \texttt{Num}, \texttt{Fractional} and \texttt{Floating} provide unary and binary numerical operations, such as arithmetic operations and trigonometric functions. However, because we want to use them with the \texttt{Dynamics} type, their implementation involve lifting. Further, the \texttt{Functor} and \texttt{Applicative} typeclasses allow us to execute this lifting, since they are designed for this purpose. The code bellow depicts the implementation for unary and binary operations, which are used in the requirements for those typeclasses:
...
After these improvements in the \texttt{Dynamics} type, it is possible to map some of them to FF-GPAC's concepts. As we will see shortly, the implemented numerical typeclasses, when combined with the lifting typeclasses (\texttt{Functor}, \texttt{Applicative}, \texttt{Monad}), express three out of four FF-GPAC's basic circuits presented in Figure \ref{fig:gpacBasic} in the previous chapter.
...
The simplest unit of all, \texttt{Constant Unit}, can be achieved via the implementation of the \texttt{Applicative} and \texttt{Num} typeclasses. First, this unit needs to receive the time of simulation at that point, which is an granted by the \texttt{Dynamics} type. Next, it needs to return a constant value $k$ for all moments in time. The \texttt{Num} given the \texttt{Dynamics} type the option of using number representations, such as the types \texttt{Int}, \texttt{Integer}, \texttt{Float} and \texttt{Double}. Further, the \texttt{Applicative} typeclass can lift those number-related functions to the desired type by using the \textit{pure} function.
...
\figuraBib{GPACBind1}{The ability of lifting numerical values to the \texttt{Dynamics} type resembles three FF-GPAC analog circuits: \texttt{Constant}, \texttt{Adder} and \texttt{Multiplier}}{}{fig:gpacBind1}{width=.9\textwidth}%
...
There are three functions that involve the \texttt{Integrator} and the \texttt{Dynamics} types together: the function \textit{newInteg}, responsible for allocating the memory that the pointer will pointer to, \textit{readInteg}, letting us to read from the pointer, and \textit{diffInteg}, a function that alters the content of the region being pointed. In summary, these functions allow us to create, read and update data from that region, if we have the pointer on-hand. All functions related to the integrator use what's known as \texttt{do-notation}, a syntax sugar of the \texttt{Monad} typeclass for the bind operator. The code bellow is the implementation of the \textit{newInteg} function, which creates an integrator:
...
The following stage is to do a type conversion, given that in order to create the \texttt{Integrator} record, it is necessary to have the type \texttt{IORef (Dynamics Double)}. At first glance, this can seem to be an issue because the result of the \textit{newIORef} function is wrapped with the \texttt{IO} monad~\footnote{\label{foot:IORef} \texttt{IORef} \href{https://hackage.haskell.org/package/base-4.16.1.0/docs/Data-IORef.html}{\textcolor{blue}{hackage documentation}}.}. This conversion is the reason why the \texttt{IO} monad is being used in the implementation, and hence forced us to implement the typeclass \texttt{MonadIO}. The function \texttt{liftIO} is capable of removing the \texttt{IO} wrapper and adding an arbitrary monad in its place, \texttt{Dynamics} in this case. So, after line 3 the \texttt{comp} value has the desired \texttt{Dynamics} type. The remaining step of this creation process is to construct the integrator itself by building up the record with the correct fields, e.g., the dynamic version of the initial value and the pointer to the constructed computation written in memory (lines 4 and 5).
...
To read the content of this region, it is necessary to provide the integrator to the $readInteg$ function. Its implementation is straightforward: build a new \texttt{Dynamics} that applies the given record of \texttt{Parameters} (line 5) to what's being stored in the region (line 4). This is accomplished by using \texttt{do-notation} with the $readIORef$ function~\footref{foot:IORef}.
