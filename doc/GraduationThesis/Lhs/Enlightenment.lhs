...
Systems of differential equations reside in the mathematical domain. In order to \textbf{execute} using the \texttt{Rivika} DSL, this model needs to be converted into an executable model following the DSL's guidelines. Further, these requirements resemble FF-GPAC's description of its basic units and rules of composition. Thus, these mappings between the three worlds need to be established. Chapters 2 and 3 explained the mapping between \texttt{Rivika} and FF-GPAC. It remains to map the \textit{semantics} of the mathematical world to the \textit{operational} world of \texttt{Rivika}. This mapping goes as the following:
...
\figuraBib{Rivika2GPAC}{The developed DSL picks a system described by differential equations and translates that to an executable model that resembles FF-GPAC's description}{}{fig:rivika2gpac}{width=.8\textwidth}%

In line 3, a record with type \texttt{Integrator} is created, with $1$ being the initial condition of the system. Line 4 creates a \textbf{state variable}, a label that gives us access to the output of an integrator, \texttt{integ} in this case. Afterward, in line 5, the \textit{diffInteg} function connects the inputs to a given integrator by creating a combinational circuit, \texttt{(y + t)}. Polynomial circuits and integrators' outputs can be used as available inputs, as well as the \textit{time} of the simulation. Finally, line 6 returns the state variable as the output for the \textbf{driver}, the main topic of the next section.

There is, however, an useful improvement to be made into the definition of a model within the DSL. The presented example used only a single state variable, although it is common to have \textbf{multiple} state variables, i.e., multiple integrators, modeling different aspects of a given scenario. Moreover, when dealing with multiple state variables, it is important to maintain \textbf{synchronization} between them, i.e., the same \texttt{Parameters} is being applied to \textbf{all} state variables at the same time.

To address both of these requirements, we will use the \textit{sequence} function, available in Haskell's standard library. This function manipulates \textbf{nested} structures and change their internal structure. The only requirement is the outer type to implement the \texttt{Traversable} typeclass. For instance, applying this function to a list of values of type \texttt{Maybe} would generate a single \texttt{Maybe} value in which its content is a list of the previous content individually wrapped by the \texttt{Maybe} type. This is only possible because the external or "bundler" type, list in this case, has implemented the \texttt{Traversable} typeclass. Figure \ref{fig:sequence} depicts the example before and after applying the function.
...
Finally, when creating a model, the same steps have to done in the same order, always starting with the integrator functions and finishing with the \textit{sequence} function being applied to a state vector. So, Figure \ref{fig:modelPipe} depicts the general pipeline used to create any model in both the semantic and operational perspectives:

\begin{figure}[H]
\begin{center}
\includegraphics[width=0.97\linewidth]{GraduationThesis/img/ModelPipeline}
\end{center}
...
On line 3, we convert the \textit{time} interval of the simulation to an \textit{iteration} interval in the format of a tuple, i.e., the continuous interval becomes the tuple $(0, \frac{stopTime - startTime}{timeStep})$. From line 4 to line 7, we are defining an auxiliary function \textit{parameterise}. This function picks a natural number, which represents the iteration index, and creates a new record with the type \texttt{Parameters}. Additionally, it uses the auxiliary function \textit{iterToTime} (line 5), which converts the iteration number from the domain of discrete \textbf{steps} to the domain of \textbf{discrete time}, i.e., the time the solver methods can operate with. This conversion is based on the time step being used, as well as which method and in which stage it is for that specific iteration. Finally, line 8 produces the outcome of the \textit{runDyanmics} function. The final result is the output from a function called \textit{map} being the argument of the \textit{sequence} function.

The \textit{map} operation is provided by the \texttt{Functor} of the list monad, and it applies an arbitrary function to the internal members of a list in a \textbf{sequential} manner. In this case, the \textit{parameterise} function composed with the dynamic application is the one being mapped, in which a custom value of the type \texttt{Parameters} is created in each iteration and this is being applied to the received \texttt{Dynamics} value. It produces a list of answers in order, each one wrapped in the \texttt{IO} monad. To abstract out the \texttt{IO}, thus getting \texttt{IO [a]} rather than \texttt{[IO a]}, the \textit{sequence} function finishes the implementation.

Additionally, there are analogous implementation of this function, so-called \textit{runDynamicsFinal}, that return only the final result of the simulation, i.e., $y(stopTime)$, instead of the outputs at the time step samples.

\section{An attractive example}

For the example walkthrough, the same example introduced in the chapter \textit{Introduction} will be used in this section. So, we will be solving a system, composed by a set of chaotic solutions, called \textbf{the Lorenz Attractor}. In these type of system, the ordinary differential equations are used to model chaotic systems, providing solutions based on parameter values and initial conditions. The original differential equations are presented bellow:
...
The first two records, \texttt{Interval} and \texttt{Solver}, sets the environment (lines 1 to 6). The former determines the simulation interval, from start to finish, and the latter configures the solver with $0.01$ as the time step, whilst executing the second-order Runge-Kutta method from the initial stage. The \textit{lorenzModel}, presented after setting the constants (lines 7 to 9), executes the aforementioned pipeline to create the model: allocate memory, create read-only pointers, change the computation and dispatch it (lines 10 to 21). Finally, the function \textit{lorenzSystem} groups everything together calling the \textit{runDynamics} driver (line 22).

After this overview, let's follow the execution path used by the compiler. Haskell's compiler works in a lazily manner, meaning that it calls for execution only the necessary parts. So, the first step calling \textit{lorenzSystem} is to call the \textit{runDynamics} function with a model, interval and solver configurations. Following its path of execution, the \textit{map} forces the application of a parametric record generated by the \textit{parameterise} function to the provided model, \textit{lorenzModel} in this case. Thus, it needs to be executed in order to return from the \textit{runDynamics} function.

To understand the model, we need to follow the execution sequence of the output: \texttt{sequence [x, y, z]}, which requires executing all the lines before this line. For the sake of simplicity, we will follow the execution of the operations related to the $x$ variable, given the remaining calls relative to the other two variables are analogous. First and foremost, memory is allocated for the integrator to work with (line 12). Figure \ref{fig:allocateExample} depicts this idea, as well as being a reminder of what the \textit{newInteg} and \textit{initialize} functions do, described in the chapter \textit{Effectful Integrals}. In this image, the integrator \texttt{integX} comprises two fields, \texttt{initial} and \texttt{computation}. The former is a simple value of the type \texttt{Dynamics Double} that, regardless of the parameters record it receives, it returns the initial value of the system. The latter is a pointer or address that references in memory a specific \texttt{Dynamics Double} computation: in the case of receiving a parameters record, it fixes potential problems with it, and it applies this fixed value in order to get \texttt{i}, i.e., the initial value $1$, the same being saved in the other field of the record.
...
The next step is the creation of the independent state variable $x$ via \textit{readInteg} function (line 15). This variable will read the computations being executing under the hood by the integrator. The core idea is to read from the computation pointer inside the integrator and create a new \texttt{Dynamics Double} value. Figure \ref{fig:readExample} portrays this mental image. When reading a value from an integrator, the computation pointer is being used to access the memory region previously allocated. Although the \textit{initialize} function is in the middle of the way, what's being stored in memory is a \texttt{Dynamics Double} value. The state variable, $x$ in this case, combines its received \texttt{Parameters} value, so-called \texttt{ps}, and \textbf{applies} it to the dynamic function. The result, \texttt{v} is then returned.
...
After establishing what the state variable $x$ is pointing to, the final image of the variable or readers is showed in Figure \ref{fig:finalModelExample}.
...
Lastly, the state variable is wrapped inside a list and it is applied the \textit{sequence} function, as explained in the previous section. This means that the list of variable(s) in the model, with the signature \texttt{[Dynamics Double]}, is transformed into a value with the type \texttt{Dynamics [Double]}. The transformation can be visually understood when looking at Figure \ref{fig:finalModelExample}. Instead of picking one \textit{ps} of type \texttt{Parameters} and returning a value \textit{v}, the same parameters record returns a \textbf{list} of values, with the \textbf{same} parametric dependency is being applied to $[x, y, z]$.

However, this only addresses \textbf{how} the driver triggers the entire execution, but does \textbf{not} explain how the differential equations are actually being calculated with the chosen numerical method, \texttt{RK2} in this example. This is done by the solver functions and, regardless of the chosen method, are all based on equation \ref{eq:solverEquation}, also introduced in chapter 3. The equation goes as the following:

$$y_{n+1} = y_n + hf(t_n,y_n) \rightarrow y_n = y_{n-1} + hf(t_{n-1}, y_{n-1})$$

The equation above makes the dependencies in the \texttt{RK2} function in Figure \ref{fig:finalModelExample} more clear:
